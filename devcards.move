module project::devcard{
    ...
    public entry fun create_card(
        ...
        let newDevCard = DevCard{
            id: id,
            name: string::utf8(name),
            owner: tx_context::sender(ctx),
            title: string::utf8(title),
            img_url: url::new_unsafe_from_bytes(img_url),
            decription: option::none(),
            years_of_exp,
        ...
        object_table::add(&mut devhub.cards, devhub.counter, newDevCard)

}

public entry fun update_card_description(devhub: &mut DevHub, new_description: vector<u8>, id: u64, ctx: &TxContext){
    let user_card = object_table::borrow_mut(&mut devhub.cards, id);
    assert!(tx_context::sender(ctx) == user_card.owner, NOT_THE_OWNER);

    let old_value = option::swap_or_fill(&mut user_card.decription, string::utf8(new_description));
...
public entry fun update_portfolio(devhub: &mut DevHub, new_portfolio: vector<u8>, id: u64, ctx: &TxContext){
    let user_card = object_table::borrow_mut(&mut devhub.cards, id);
    assert!(tx_context::sender(ctx)== user_card.owner, NOT_THE_OWNER);
...
public entry fun deactivate_card(devhub: &mut DevHub, id: u64, ctx: &TxContext){
    let user_card = object_table::borrow_mut(&mut devhub.cards, id);
    assert!(tx_context::sender(ctx) == user_card.owner, NOT_THE_OWNER);
...
public fun get_card_info(devhub: &DevHub, id: u64): (String, address, String, Url, Option<String>, u8, String, String, String, bool){
    let user_card = object_table::borrow(&devhub.cards, id);
    (
        user_card.name,
        user_card.owner,
        user_card.title,
        user_card.img_url,
        user_card.decription,
        user_card.years_of_exp,
        user_card.technologies,
        user_card.portfolio,
        user_card.contact,
        user_card.open_to_work,
    )
}
